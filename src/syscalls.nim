...
proc NtOpenThread*(ThreadHandle: PHANDLE, DesiredAccess: ACCESS_MASK, ObjectAttributes: POBJECT_ATTRIBUTES, ClientId: PCLIENT_ID): NTSTATUS {.asmNoStackFrame.} =
    asm """
	mov [rsp +8], rcx          
	mov [rsp+16], rdx
	mov [rsp+24], r8
...
proc NtQueueApcThread*(ThreadHandle: HANDLE, ApcRoutine: PVOID, ApcArgument1: PVOID, ApcArgument2: PVOID, ApcArgument3: PVOID): NTSTATUS {.asmNoStackFrame.} =
    asm """
	mov [rsp +8], rcx          
	mov [rsp+16], rdx
	mov [rsp+24], r8
